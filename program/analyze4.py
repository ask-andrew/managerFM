...
def analyze_email_interactions(emails_data, user_email):
    ...
    """
    # This will count total emails where the contact was involved (sent or received by user)
    ...
    for thread_id, email_list in threads.items():
        ...
        # Track if the user has sent *any* email in this specific thread
        user_sent_any_email_in_thread = False

        for email_entry in sorted_emails:
            email_date_obj = parse_email_date(email_entry['date'])
            ...
            # Count emails exchanged for each contact
            # This counts every email where the contact is either sender or recipient (with user involvement)
            if is_sent_by_user:
            ...
            # If this email is sent by the user AND there was a previous incoming email in this thread
        ...
        # Sort descending to easily get the latest email
        sorted_emails_desc = sorted(email_list, key=lambda x: parse_email_date(x['date']) if parse_email_date(x['date']) else datetime.datetime.min, reverse=True)
        
        if sorted_emails_desc:
            latest_email_in_thread = sorted_emails_desc[0]
            latest_email_date_obj = parse_email_date(latest_email_in_thread['date'])
            ...
            if latest_email_date_obj:
                ...
                # Condition for awaiting response:
                # 1. The latest email in the thread was received by the user.
                # 2. The latest email was NOT sent by the user.
                # 3. The user has NOT sent any email in this thread *after* this latest incoming email.
                # 4. The email is within a reasonable time window (e.g., older than 1 hour, younger than 14 days).

                if is_latest_received_by_user and not is_latest_sent_by_user:
                    ...
                    for email_in_thread in sorted_emails: # Iterate through all emails in original order
                        email_in_thread_date_obj = parse_email_date(email_in_thread['date'])
                    ...
                    if not user_replied_after_this_incoming:
    ...
    # top_email_exchange_contacts will now be based on email_exchange_counts
...
def get_consolidated_contacts_summary(key_people_combined, top_email_exchange_contacts, avg_response_times, name_to_email_map, user_email):
    ...
    consolidated_data = defaultdict(lambda: {
        ...
        'emails_exchanged': 0, # Emails sent/received (from email_exchange_counts)
    ...
    # Sort by interactions (mentions) in descending order, then by emails_exchanged
...
def format_brief_as_html(key_people, key_organizations, themes, chart_files, consolidated_contacts_summary, time_window_days, start_date, end_date, all_processed_emails_data, logo_cid, thread_id_to_simplified_topic_map, llm_digest):
    """
...
async def generate_manager_briefing_data():
    ...
    try:
        ...
        for email_entry in all_processed_emails_data:
        ...
        consolidated_contacts_summary = get_consolidated_contacts_summary(key_people_combined, top_email_exchange_contacts, avg_response_times, name_to_email_map, SENDER_EMAIL_ADDRESS)

        # --- Combine all data into a single structure for LLM input ---
        # This is the crucial part: provide all necessary data for the LLM to generate its HTML
        llm_input_data = {
            ...
            "key_organizations": key_organizations_combined.most_common(10), # Pass top 10
            "top_themes_keywords": themes # Pass top themes
        }

        # --- Generate LLM Digest ---
        llm_prompt = load_prompt_from_file(GEMINI_PROMPT_FILE_PATH)
        llm_digest = "<!-- Error: Digest could not be generated. Please check prompt and API key. -->"
        if llm_prompt:
            llm_digest = await generate_digest_with_gemini(llm_input_data, llm_prompt)
        ...
        html_email_body = format_brief_as_html(
            ...
            consolidated_contacts_summary, # Still pass for console output if needed
            time_window_days, start_time, end_time, all_processed_emails_data, logo_cid,
            thread_id_to_simplified_topic_map, # Still pass for console output if needed
            llm_digest # Pass the generated digest (expected to be full HTML)
        )
        ...
        print("\nAI-Generated Digest (Full HTML - truncated for console):")
        print(llm_digest[:1000] + "..." if len(llm_digest) > 1000 else llm_digest) # Print truncated HTML for console
        print("\nKey Contacts & Response Times (Data used by LLM):")
        ...
        print("\nEmails Awaiting Your Response (Data used by LLM):")
        if emails_awaiting_response:
            for email_item in emails_awaiting_response:
                print(f"- Subject: {email_item['subject']}, From: {email_item['sender']}, Date: {email_item['date']}")
        else:
            print("- None")

        print("\nUpcoming Meetings (Data used by LLM):")
        ...
        print("\nTop 10 Key Organizations/Projects (Data used by LLM):")
        for org, count in key_organizations_combined.most_common(10):
            print(f"- {org} ({count} mentions)")
        print("\nTop 20 Themes/Keywords (Data used by LLM):")
        for theme in themes:
            print(f"- {theme}")
